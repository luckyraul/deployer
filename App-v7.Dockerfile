...
ENV DEBIAN_FRONTEND=noninteractive VAULT_VERSION=2.0.0 NOMAD_VERSION=2.0.0 LEVANT_VERSION=0.4.0 NOMADPACK_VERSION=0.4.2
...
RUN apt-get -qqy install curl wget rsync \
  && wget -qO- https://deb.nodesource.com/setup_26.x | bash - \
...
RUN apt-get -qqy install curl apt-transport-https lsb-release ca-certificates \
  ...
  && apt-get -qq update \
  && apt-get -qqy install php8.5-cli php8.5-curl php8.5-intl php8.5-xml php8.5-mbstring php8.5-gd php8.5-zip php8.5-imagick \
  && apt-get clean \
